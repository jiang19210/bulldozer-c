...
function handlerResult(err, body, callback, res, httpcontext, m) {
    try {
        ...
        if (err) {
            console.info('[%s]-发生异常.', m);
        } else {
            let result = JSON.parse(body);
            if (result.is_success && result.result == '0') {
                console.info('[数据保存失败，影响行数为0][%s##%s][%s]', m, name, JSON.stringify(httpcontext));
                ...
                console.info('[数据保存发生异常][%s##%s] result:%s', m, name, JSON.stringify(result.result));
            }
        }
        if (callback) {
