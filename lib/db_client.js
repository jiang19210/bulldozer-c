...
function handlerResult(err, body, callback, res, httpcontext, method) {
    try {
        ...
        let result = null;
        if (err) {
            console.info('[%s]-发生异常.', method);
        } else {
            result = JSON.parse(body);
            if (result.is_success && result.result == '0') {
                console.info('[数据保存失败，影响行数为0][%s##%s][%s]', method, name, JSON.stringify(httpcontext));
            } else if (result.is_success) {
                debug('[数据保存成功][%s##%s], result:%s', method, name, JSON.stringify(result.result));
            } else {
