...
function handlerResult(err, body, callback, res, httpcontext, m) {
    try {
        ...
        try {
            ...
            console.warn('获取name发生异常.', e);
        }
        if (err) {
            console.warn('[%s]-发生异常.', m);
        } else {
            ...
            if (result.is_success && result.result == '0') {
                ...
                console.warn('[数据保存发生异常][%s##%s] result:%s', m, name, JSON.stringify(result.result));
            }
        }
        if (callback) {
        ...
        console.warn('handlerResult发生异常.%s. %s', body, e);
    }
}
